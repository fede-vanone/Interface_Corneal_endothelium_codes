...
% second derivative of the chloride concentration in the cleft
...
overall_water_flux=(qtj*Ux*h+Qca)/(w+h); % dimensional transendothelial 
% water flux towards the apical compartment, in m^2/s

% check conservation of water (dimensionless)
Water_cons=qin-2*qlat-qtj; 


%% ion conservations (dimensionless)
...
% check chloride conservation in the cleft
% J_cl_in_nd-J_cl_lat_nd-J_cl_out_nd

u_slip_new=-bar_sigma.*y8./sqrt(2*(y1+y3))-bar_sigma.^2.*(y2+y4)./16./(y1+y3).^2;
press_eo=mu*u_slip*Ux*L/h^2; 
press_eo_tj=epsilon*y7(end)^2*R*T/F/L^2;


% flux accounting for hexagonal cells
